...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($peso)) {
        $errores["peso"] = "Introduce tu peso.";
    }
    if (empty($sexo)) {
        $errores["sexo"] = "Introduce tu sexo.";
    }
    if (empty($civil)) {
        $errores["civil"] = "Selecciona tu estado civil.";
    }
    if (empty($aficio)) {
        $errores["aficio"] = "Selecciona al menos una afición.";
    }
...
<form method="POST" action="index.php">
    ...
    <input type="radio" name="civil" value="Otros" <?= $civil=="Otros"?"checked":"" ?>> Otro:
    <input type="text" name="otroEst" placeholder="Escribe aquí..." size="25" value="<?=$otroEstado) ?>"><br>
