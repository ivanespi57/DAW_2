<?php
// API REST para tareas (migrada desde tasks.php)
...
switch ($method) {
    ...
    case 'PATCH':
        ...
        if (empty($fields)) { http_response_code(400); echo json_encode(['error' => 'No hay campos para actualizar']); exit; }
    ...
    case 'DELETE':
        if (!$id) { http_response_code(400); echo json_encode(['error' => 'ID de tarea requerido']); exit; }
        try {
            $stmt = $pdo->prepare("SELECT * FROM tasks WHERE id = ?");
            $stmt->execute([$id]);
            $task = $stmt->fetch();
            if (!$task) { http_response_code(404); echo json_encode(['error' => 'Tarea no encontrada']); exit; }
            $stmt = $pdo->prepare("DELETE FROM tasks WHERE id = ?");
            $stmt->execute([$id]);
            echo json_encode(['message' => 'Tarea eliminada correctamente', 'deleted_task' => $task]);
        } catch (PDOException $e) {
            http_response_code(500);
