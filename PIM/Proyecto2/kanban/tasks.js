...
class TaskAPI {
    ...
    // ValidaciÃ³n bÃ¡sica del contenido de una tarea
    static validateTask(description, status, priority) {
        if (!description || typeof description !== 'string'){
            throw new Error('La descripciÃ³n debe ser un texto vÃ¡lido.');
        }
        ...
        if (status && !validStatus.includes(status)) {'top'
            throw new Error(`Estado invÃ¡lido. Valores permitidos: ${validStatus.join(', ')}`);
        }

        const validPriority = ['low', 'medium', 'high','top'];
        if (priority && !validPriority.includes(priority)) {
            throw new Error(`Prioridad invÃ¡lida. Valores
            permitidos: ${validPriority.join(', ')}`);
        }
    }

    // FunciÃ³n comÃºn para manejar errores de Fetch y HTTP
    static async handleResponse(response) {
        if (!response.ok) {
            // Intentamos obtener JSON con error del backend
            const errorText = await response.text();
            throw new Error(`Error HTTP ${response.status}:${errorText || 'Respuesta no vÃ¡lida'}`);
        }
